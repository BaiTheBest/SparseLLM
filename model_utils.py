...
import math
import copy
from transformers import OPTForCausalLM, LlamaForCausalLM, AutoModelForCausalLM, AutoTokenizer
...
def get_qwen(args):
    ...
    # Set sequence length based on configuration
    # model.seqlen = model.config.max_position_embeddings if hasattr(model.config, 'max_position_embeddings') else 2048
...
@torch.no_grad()
def qwen_sparsellm(model, dataloader, dev, args):
    print("Starting...")
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            # 捕获输入和 attention_mask
            inps[cache["i"]] = inp
            cache["i"] += 1
            cache["attention_mask"] = kwargs["attention_mask"]
            raise ValueError  # 用于中断 forward

    # 替换第一个 Transformer 层为捕获器
    layers = model.model.layers
    layers[0] = Catcher(layers[0])

    # 运行数据加载器，确保 batch 在 GPU 上
    for batch in dataloader:
        try:
            batch = batch[0].to(dev)  # 将 batch 移到 GPU
            model(batch)  # 前向传递
        except ValueError:
            pass  # 捕获输入后退出
